...
export const createCustomer = async ({ merchantId, payload }) => {
  ...
  const customerId = await sequelize.transaction(async (t) => {
    ...
    let exists = await Customer.findOne({ where: { customerCode }, transaction: t });

    while (exists) {
      customerCode = generateCustomerCode();
      exists = await Customer.findOne({ where: { customerCode }, transaction: t });
    }
...
export const updateCustomer = async ({ merchantId, customerId, payload }) => {
  await sequelize.transaction(async (t) => {
    ...
    if (payload.metadata && typeof payload.metadata === "object") {
      ...
      if (existingMetadata) {
        await existingMetadata.update({ metadata: payload.metadata }, { transaction: t });
      } else {
        await CustomerMetadata.create(
          {
            customerId,
